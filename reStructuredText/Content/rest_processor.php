...
/** 
 * \brief Prints an article as reStructuredText.
 *
 * \param &$article An article object with a 'flat' hierarchy.
 *
 */
function myPrintArticleReST(&$article)
{
  if (is_array($article->description)) {
    $description = myPrintTextRunAsReST($article->description);
	echo <<<EOT
$description
EOT;
  }
		...
		switch ($step->media->type)
    {
      ...
      default:
        ...
        if ($hasImage)
        {
          ...
          $mediaAlt = myPrepareStringForReST($step->media_alt);
...
EOT;
		...
		// Start Output
		$stepContent = '';
		$title = myPrintTextRunAsReST($step->title, 'title');
		$instructions = myPrintTextRunAsReST($step->instructions);
	
		// Title
		if (!empty($title))
		{
			ob_start();
			$underline = str_repeat($stepTag,strlen($title));
			echo <<<EOT
{$title}
{$underline}
EOT;
...
/**
 * \brief Prints a text run as reStructuredText
 *
 * \param $textRun
 * \param $type 'instructions' or 'title'.
 */
function myPrintTextRunAsReST($textrun, $type='instructions') {
  $output = '';
  $lastStyle = -1;
      ...
      {      	
        case 'decimal':
          if ($para->style->list_depth > 1)
	          $output .= str_repeat('#.', ($para->style->list_depth)-1);
	        $output .= '#. ' ;
          break;
        default:
        	if ($para->style->list_depth > 1)
      ...
      foreach ($para->runs as $run)
      {
        ...
        $styles = explode(',', $run->style->font_styles);
         ...
         if (property_exists($run->style, 'link') && is_object($run->style->link) && !empty($run->style->link->url)) {
          if (strpos($run->style->link->url, 'http') !== FALSE) {
              $prefix = chr(96);
              $suffix = ' ';
              $linksuffix = '_';
          } else {
          ...
          $closingRun  = '<' . $run->style->link->url . '>'. chr(96) . $linksuffix;
        }
        ...
        */
        ...
        $output .= $prefix;
        $output .= myPrepareStringForReST($run->text, $para->metadata->style);
        $output .= $suffix;
        $output .= $closingRun;
      }
    }
 ...
 *
 */
function myPrepareStringForReST($str, $metadata_style='')
{
